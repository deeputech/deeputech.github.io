...
Each metric has two thresholds: a **warning line** (the harness calls it _advisory_ — the run passes but prints a banner) and a **hard-fail line** (_catastrophic_ — the run exits non-zero). That is the "warning line" and "hard-fail line" I refer to throughout.
...
- **The wrapper is honest.** LlamaStash spawns the same binary you would run by hand, with the same flags. With matched flags there is no measurable throughput cost — on AMD APU, Apple Silicon, and NVIDIA, on every metric.
...
You can follow me on [Bluesky](https://bsky.app/profile/deepu105.bsky.social), [X](https://x.com/deepu105), [Mastodon](https://mastodon.social/@deepu105), and [LinkedIn](https://www.linkedin.com/in/deepu05/).
